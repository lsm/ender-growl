 ...
 *
 * Growl for ender
 * https://github.com/zir/ender-growl
 *
 * Copyright (c) 2011 Senmiao Liu
 * MIT License
 *
 * Version: 0.5.0
 */
...
(function($) {
  ...
  var Gritter = {
    ...
    add: function(params) {
      ...
      // Assign callbacks
      $(['before_open', 'after_open', 'before_close', 'after_close']).each(function(val, idx) {
      ...
      item.css({'display': null, opacity: 0});

      var itemAnimator = item.fadeIn(this.fade_in_speed, function() {
      ...
      item.on('mouseenter mouseleave', function(event) {
        ...
        if (event.type === 'mouseover') {
          type = 'mouseenter';
          !sticky && Gritter._restoreItemIfFading(item, number, itemAnimator);
        } else {
          !sticky && Gritter._setFadeTimer(item, number);
        }
        Gritter._hoverState(item, type);
      });
     ...
     * @param {Object} animator Object use to stop the animation
     */
    _restoreItemIfFading: function(e, unique_id, animator) {
...
//      e.css({ opacity: null });
    ...
    stop: function(params) {
      ...
      before_close(wrap);
      wrap.fadeOut(this.fade_out_speed, function() {
        wrap.remove();
        after_close();
      });
